...
let run unit =
    assertCorpus(
        [
            ...
            "a + b * c << 2 == d",
            "head :: middle :: []",
            "value |> map(f) |?> finish |!> recover",
            "Module.Submodule.binding",
            "f()(1, 2)([3, 4])",
            "Point(x = 1, y = 2)",
            "map transform values",
            "(1, \"two\", '三')",
            "await task(Unit)",
            "perform Clock.now(Unit)",
            "if ready then run(Unit) else stop(Unit)",
            "given (left, right) -> left + right",
            "let recursive loop value = loop(value) in loop(0)",
            "let? value = result in value",
            "let! value = task in value",
            "match value with | Some(inner) when inner > 0 -> inner | None -> 0",
            "handle Clock.now(Unit) with | Clock.now(value) -> value | return(value) -> value",
            "point with x = 1, y = 2",
            "@",
            "(1, 2",
            "[1, 2",
