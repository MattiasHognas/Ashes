// A callee's pair of records is released at the back edge while one of its records is consed
// onto an accumulator in one branch of a let-bound choice: the accumulator is placed on the
// reference-counted heap, whose back edge copies the successor with its own references first.
// expect: 882909
type Tok =
    | kind: Int
    | text: Str
