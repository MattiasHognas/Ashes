...
| **Windows readiness TCP stepping** | The Windows TCP connect/send/receive leaf-step helpers now preserve per-task progress, switch sockets into non-blocking mode, and resume through `WSAPoll` readiness waits so the same pending-wait model can also drive TLS handshakes and encrypted send/receive stages. |
