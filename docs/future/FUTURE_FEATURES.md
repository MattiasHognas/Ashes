...
| [HTTPS/TLS](HTTPS_TLS.md) | Partial | Transparent `https://` in `Ashes.Http` and the public `Ashes.Net.Tls` surface now ride on the hermetic `rustls` runtime; broader backend coverage and final cleanup remain |
