...
| Backend | LLVM C API bindings only so far (`selfhost/packages/backend`, `AshesCompiler.Backend.Llvm`): context/module/builder/type/value/basic-block creation, `functionType`, `addFunction`, `appendBasicBlock`, `constInt`, `buildRet`, `verifyModule`, x86 target initialization, `getTargetFromTriple`, `createTargetMachine`, `applyDataLayout`, and `targetMachineEmitToMemoryBuffer`, covering just enough of `src/Ashes.Backend/Llvm/Interop/LlvmApi.cs`'s surface to build and verify a trivial function in a real LLVM module and emit it to a genuine linux-x64 ELF relocatable object end to end — verified by actually running the compiled test binary with a `libLLVM` build placed next to it and checking the emitted object with `readelf`, not just compiling it or asserting a byte length. No host-CPU detection or optimization-level selection yet, and IR-to-LLVM codegen itself, linking, the other three target RIDs, and the rest of the LLVM C API surface are unported |
