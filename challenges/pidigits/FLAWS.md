...
- **Growth / arena churn (the perf probe).** `q`, `r`, `t` grow without bound; every step
  ...
  allocation volume and by the division algorithm. **Update:** the per-iteration *intermediate*
  `BigInt` garbage is now reclaimed — a `BigInt` is a self-contained buffer, so it is copied out
  across the TCO reset like a `String`, and the reset fires (resident set `N=1000` 168 MB → 60 MB at
  the same time). What remains is the *growing accumulator* itself (`q, r, t` widen each step and the
  whole-value copy is preserved below the advancing watermark), giving the residual `O(N²)` memory —
  the ownership / in-place-reuse memory-model milestone, shared with fannkuch's growth.
