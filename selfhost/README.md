...
|---|---|
| Package contracts | Complete portable source archives for frontend, formatter, semantics, and (so far, `add`/`fmt`/`init`/`remove`/`tree`/`why` only) cli, plus backend (the LLVM bindings, ELF linker, and the IrCodegen dotted-filename slices: Support/Filesystem/TextBytes plus the core dispatcher), with versioned registry dependencies, exact locks, and declared local overrides |
