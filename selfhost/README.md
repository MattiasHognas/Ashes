...
|---|---|
| Package contracts | Complete portable source archives for frontend, formatter, semantics, and (so far, `add`/`fmt`/`init`/`remove`/`tree`/`why` only) cli, plus backend (the LLVM bindings, ELF linker, and the IrCodegen dotted-filename slices: Support/Arena/Filesystem/TextBytes/Environment/Process plus the core dispatcher, with the linux-x64 syscall wrappers isolated in the platform-suffixed Syscalls.LinuxX64 slice), with versioned registry dependencies, exact locks, and declared local overrides |
