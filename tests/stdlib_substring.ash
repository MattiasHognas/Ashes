// Ashes.Text.substring is codepoint-indexed. It was take(drop(...)) — an O(start + count^2)
// char-walk with per-char string concatenation, so a sliding k-mer window was catastrophic
// (8000 chars -> ~63 s). It now does a single codepoint->byte offset walk + Ashes.Byte.subText
// (O(start + count), no concatenation; ~2000x faster on that window), staying UTF-8 correct.
// For byte-indexed O(1) slicing, Ashes.Byte.subText remains the fast path.
