...
let compareMethod unit =
    TraitMethodDecl(name = "compare", signature = TypeArrow(TypeNamed("a"))(TypeArrow(TypeNamed("a"))(TypeNamed("Int"))([])(None))([])(None), defaultImplementation = None)

let equalTrait unit = TraitDecl(name = "Equal", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [equalMethod(Unit)])

let orderedTrait unit = TraitDecl(name = "Ordered", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed("a")])], methods = [compareMethod(Unit)])

let equalIntImplementation unit =
    TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed("Int")], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = ExprLambda("left")(ExprLambda("right")(ExprBool(true))(None))(None))])
...
let evidenceEnvironment unit =
    ...
    |> TopLevelImplementation], body = None)) with
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } -> test.fail("trait evidence program should infer: " + Ashes.Trait.Show.show(error))

let expectConcreteEvidenceArguments unit =
    match Unit
    |> evidenceEnvironment
    |> planTraitEvidenceArguments([TraitConstraint(traitName = "Ordered", typeArguments = [SemInt]), TraitConstraint(traitName = "Equal", typeArguments = [SemInt])]) with
