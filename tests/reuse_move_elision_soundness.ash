...
// (used once per path, seeded from the sole-nullary `Ashes.Collection.Map.empty`), and `let base = ...` passes
// `Ashes.Collection.Map.empty` directly. Elision makes the nested re-entry (`outer` calling `setFold` per batch)
...
let base = setFold(0)(20)(Ashes.Collection.Map.empty)
