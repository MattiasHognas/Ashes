...
**Phase 3 — Closure result/argument provenance unification (Medium complexity).** Fold
`_functionReturnedClosureLabels`/`_runtimeManagedFunctionResultLabels`/`_knownFunctionLabelsBySlot`
into the extended summary (§5 item 3). **Update (landed, PR pending): measured real effect on
fannkuch-redux N=11 is small (~45 GB → ~44 GB, ~2%), not the dramatic fix once hoped for.** An
earlier, separate, never-landed experimental fix (discarded diff, see §7's discrepancy note) had
suggested a much larger 47 GB → 27.4 GB improvement from this general mechanism; Phase 3's safety-
reviewed implementation (which deliberately only ever promotes a value to RC when *provably* safe,
never demotes an existing safe answer — the correct discipline for this code) does not reproduce
that number. Root-caused directly: `FunctionResultProvenance`'s "forwards to another function"
classification answers "does the *whole result* forward," not "is *this constructor argument*
fresh when it comes from calling another user function" — the latter is a genuinely different,
inter-procedural question (`IsFreshConstructionArgument`'s domain) that would need each callee's own
provenance consulted recursively through call cycles (rotateFirst/setAt/insertAt/nextPerm all call
each other in the fannkuch accumulator chain) — real, separate work, adjacent to Phase 6/`GetAdtField`
territory, correctly deferred rather than chased under this phase's scope. **fannkuch-redux's actual
dominant leak driver is confirmed to be the "third gap" flagged in the original fannkuch
investigation**: `GetAdtField`/match-extracted-field provenance in `Lowering.Patterns.cs` (a
`match st with | S(perm, count) -> ...`-shaped extraction, not a closure/forwarding-chain question) —
this remains unfixed and is not part of Phase 3, 4, or 5 as currently scoped; whoever tackles it
should treat it as its own investigation, likely adjacent to Phase 6.
...
|---|---|---|
| 3 (closures) | fannkuch-redux investigation (this session) | Silent leak from unrecognized-already-RC value — mechanism confirmed, but not fannkuch's dominant driver (see §7) |
...
**Discrepancy resolved (was open, now settled by Phase 3's actual measurement)**: PR #299's own
...
(discarded diff) had instead pointed at the closure/function-result provenance gap and measured a
47GB→27.4GB improvement from it. **Phase 3's formal, safety-reviewed implementation of that same
general mechanism does not reproduce anywhere near that improvement** (real measured effect: ~45GB→
~44GB, ~2%) — see Phase 3's updated description in §6. The free-list theory was never independently
re-verified and is now considered unlikely to have been the dominant cause; the actual dominant
driver, confirmed by direct `FunctionResultProvenance` inspection against the real fannkuch program,
is the match-extracted-field provenance gap (`GetAdtField` in `Lowering.Patterns.cs`) — a
`match`-destructuring question, not a closure-forwarding or allocator question. This remains
unfixed. Whoever picks it up next should start from Phase 3's PR discussion (direct
`ASHES_EXPLAIN_OWNERSHIP=all` trace of `nextPerm`/`rotateFirst`/`setAt`/`insertAt`/`loop` on the real
...
starting point: mechanical, narrow blast radius, and each gives a real (if in Phase 3's case modest)
measured improvement while proving the "unified ownership fact, consulted not re-derived" pattern.
fannkuch-redux's dominant remaining leak is now confirmed to be a separate, unscoped gap (§7) —
Phases 4-6 are where the bulk of the risk and effort live, and should
