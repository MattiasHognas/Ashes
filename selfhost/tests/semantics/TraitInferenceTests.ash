...
let ordDeclaration =
    TraitDecl(name = "Ordered", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed(
        "a"
    )])], methods = [TraitMethodDecl(name = "compare", signature = TypeArrow(
        TypeNamed("a"),
...
let expectTraitRegistration unit =
    ...
    |> Some)) with
        | ProgramInferenceResult { semanticType = semanticType, substitution = substitution, environment = environment, error = None } ->
            let methodTypeChecked =
                match applySubstitution(substitution)(semanticType) with
                    | SemFunction(SemVariable(leftId), SemFunction(SemVariable(rightId), SemBool, None), None) ->
                        test.assertEqual(
                            leftId,
                            rightId
                        )
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
            test.fail(
                "trait declaration should infer: " + Ashes.Trait.Show.show(error)
            )

let expectForwardSupertraitRegistration unit =
    match inferProgram(
        ProgramSyntax(items = [TopLevelTrait(ordDeclaration), TopLevelTrait(eqDeclaration)], body = None)
    ) with
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
            test.fail(
                "forward supertraits should register: " + Ashes.Trait.Show.show(error)
            )

let expectInvalidTraitDeclarations unit =
    (let duplicateChecked =
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelTrait(eqDeclaration)], body = None)
        ) with
    ...
    in
        let emptyParameters =
            TraitDecl(name = "Empty", typeParameters = [], supertraits = [], methods = [TraitMethodDecl(name = "value", signature = TypeNamed(
                "Int"
            ), defaultImplementation = None)])
        in
            ...
            in
                let duplicateParameters =
                    TraitDecl(name = "Duplicate", typeParameters = [
                        TypeParameter(name = "a"),
                        TypeParameter(name = "a")
                    ], supertraits = [], methods = [equalMethod])
                in
                    let duplicateParametersChecked =
                        match inferProgram(
                            ProgramSyntax(items = [TopLevelTrait(duplicateParameters)], body = None)
                        ) with
                    ...
                    in
                        let duplicateMethods =
                            TraitDecl(name = "Duplicate", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [
                                equalMethod,
                                equalMethod
                            ])
                        in
                            let duplicateMethodsChecked =
                                match inferProgram(
                                    ProgramSyntax(items = [TopLevelTrait(duplicateMethods)], body = None)
                                ) with
                            ...
                            in
                                let detachedMethod =
                                    TraitDecl(name = "Detached", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [TraitMethodDecl(name = "value", signature = TypeNamed(
                                        "Int"
                                    ), defaultImplementation = None)])
                                in
                                    match inferProgram(
                                        ProgramSyntax(items = [TopLevelTrait(detachedMethod)], body = None)
                                    ) with
...
let expectInvalidSupertraits unit =
    (let unknown =
        TraitDecl(name = "Ordered", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "Missing", typeArguments = [TypeNamed(
            "a"
        )])], methods = [equalMethod])
    in
        let unknownChecked =
        ...
        in
            ...
            in
                let arityChecked =
                    match inferProgram(
                        ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelTrait(badArity)], body = None)
                    ) with
                ...
                in
                    let first =
                        TraitDecl(name = "First", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "Second", typeArguments = [TypeNamed(
                            "a"
                        )])], methods = [equalMethod])
                    in
                        let second =
                            TraitDecl(name = "Second", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "First", typeArguments = [TypeNamed(
                                "a"
                            )])], methods = [equalMethod])
                        in
                            match inferProgram(
                                ProgramSyntax(items = [TopLevelTrait(first), TopLevelTrait(second)], body = None)
                            ) with
...
let expectDefaultMethodValidation unit =
    (let forwardedEqual =
        ExprLambda(
            ...
            ExprLambda(
                "right",
                ExprCall(
                    ExprCall(ExprQualifiedVar("Equal")("equal"))(ExprVar("left"))(false)(callArgumentsInline),
                    ExprVar("right"),
                    false,
                    callArgumentsInline
                ),
    ...
    in
        let valid =
            TraitDecl(name = "Equal", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [equalMethod, TraitMethodDecl(name = "same", signature = TypeArrow(
                TypeNamed("a"),
                TypeArrow(TypeNamed("a"))(TypeNamed("Bool"))([])(None),
                [],
                None
            ), defaultImplementation = Some(forwardedEqual))])
        in
            let validChecked =
                match inferProgram(ProgramSyntax(items = [TopLevelTrait(valid)], body = None)) with
                    ...
                    | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                        test.fail(
                            "well-typed defaults should infer: " + Ashes.Trait.Show.show(error)
                        )
            in
                let invalid =
                    TraitDecl(name = "Rendered", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [TraitMethodDecl(name = "render", signature = TypeArrow(
                        TypeNamed("a"),
                        TypeNamed("Str"),
                        [],
                        None
...
let choiceMethod name dependency =
    ...
    |> ExprLambda(
        "value",
        ExprCall(ExprQualifiedVar("Choice")(dependency))(ExprVar("value"))(false)(callArgumentsInline)
    )
    |> Some)

let choiceBaseMethod =
    TraitMethodDecl(name = "base", signature = TypeArrow(
        TypeNamed("a"),
        TypeNamed("Bool"),
        [],
        None
...
let rejectsDefaultMethodDependencyCycle unit =
    (let choice =
        TraitDecl(name = "Choice", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [choiceMethod(
            "first",
            "second"
        ), choiceMethod(
            "second",
    ...
    in
        let implementation =
            TraitImplementationDecl(traitName = "Choice", typeArguments = [TypeNamed(
                "Int"
            )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "base", implementation = choiceImplementation)])
        in
            match inferProgram(
                ProgramSyntax(items = [TopLevelTrait(choice), TopLevelImplementation(implementation)], body = None)
            ) with
                ...
                | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                    test.fail(
                        "unexpected default cycle result: " + Ashes.Trait.Show.show(error)
                    )
...
let rejectsSelfDependentDefaultMethod unit =
    ...
    in
        ...
        in
            match inferProgram(
                ProgramSyntax(items = [TopLevelTrait(
                    recursiveChoice
                ), TopLevelImplementation(implementation)], body = None)
            ) with
                | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(DefaultTraitMethodDependencyCycle("Choice", "first")) } -> Unit
...
let acceptsOverrideBreakingDefaultMethodCycle unit =
    ...
    in
        let implementation =
            TraitImplementationDecl(traitName = "Choice", typeArguments = [TypeNamed(
                "Int"
            )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "first", implementation = choiceImplementation)])
        in
            match inferProgram(
                ProgramSyntax(items = [TopLevelTrait(choice), TopLevelImplementation(implementation)], body = None)
            ) with
...
let genericEqualImplementation =
    TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeApplied(
        "List",
        [TypeNamed("a")]
    )], requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed(
        "a"
    )])], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])

let expectTraitImplementationRegistration unit =
    match inferProgram(
        ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(
                            ...
                            match methods with
                                | TraitImplementationMethodInferenceDefinition { name = "equal", implementation = _implementation, semanticType = methodType } :: [] ->
                                    match methodType with
                                        ...
                                        | _ ->
                                            test.fail(
                                                "implementation method should use the specialized trait signature"
                                            )
                                | _ -> test.fail("generic implementation should retain its supplied method")
                | _implementations ->
                    test.fail(
                        "generic implementation should register its rigid head, requirement, and specialized method"
                    )
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
            test.fail(
                "generic implementation should infer: " + Ashes.Trait.Show.show(error)
            )
...
let expectTraitImplementationDefaults unit =
    (let defaultedMethod =
        TraitMethodDecl(name = "same", signature = TypeArrow(
            ...
            [],
            None
        ), defaultImplementation = Some(trueEqualImplementation))
    in
        let defaultedTrait =
            TraitDecl(name = "Comparable", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [TraitMethodDecl(name = "equal", signature = TypeArrow(
                TypeNamed("a"),
                TypeArrow(TypeNamed("a"))(TypeNamed("Bool"))([])(None),
                [],
                None
            ), defaultImplementation = None), defaultedMethod])
        in
            let inherited =
                TraitImplementationDecl(traitName = "Comparable", typeArguments = [TypeNamed(
                    "Int"
                )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
            in
                let overridden =
                    TraitImplementationDecl(traitName = "Comparable", typeArguments = [TypeNamed(
                        "Str"
                    )], requirements = [], bindings = [
                        TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation),
                        TraitImplementationMethodBinding(methodName = "same", implementation = trueEqualImplementation)
                    ])
                in
                    match inferProgram(
                        ProgramSyntax(items = [TopLevelTrait(
                            defaultedTrait
                        ), TopLevelImplementation(inherited), TopLevelImplementation(overridden)], body = None)
                    ) with
                        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = environment, error = None } ->
                            match resolveTraitImplementations("Comparable")(environment) with
                                | TraitImplementationInferenceDefinition { traitName = "Comparable", typeArguments = SemString :: [], requirements = [], methods = _overriddenMethods } :: tail ->
                                    match tail with
                                        ...
                                        | _implementations ->
                                            test.fail(
                                                "default methods should be optional and overridable"
                                            )
                                | _implementations -> test.fail("default methods should be optional and overridable")
                        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                            test.fail(
                                "defaulted implementations should infer: " + Ashes.Trait.Show.show(error)
                            ))

let rejectUnknownTraitImplementation unit =
    (let implementation =
        TraitImplementationDecl(traitName = "Missing", typeArguments = [TypeNamed(
            "Int"
        )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
    in
...
let rejectTraitImplementationArity unit =
    (let implementation =
        TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed("Int"), TypeNamed(
            "Str"
        )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(implementation)], body = None)
        ) with
...
let rejectUnknownTraitImplementationMethod unit =
    (let implementation =
        TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
            "Int"
        )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "other", implementation = trueEqualImplementation)])
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(implementation)], body = None)
        ) with
...
let expectInvalidTraitImplementationRequirements unit =
    (let unknownRequirement =
        TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeApplied(
            "List",
            [TypeNamed("a")]
        )], requirements = [TraitConstraintSyntax(traitName = "Missing", typeArguments = [TypeNamed(
            "b"
        )])], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
    in
        let unknownChecked =
        ...
        in
            let badArity =
                TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
                    "Int"
                )], requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [])], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
            in
                let arityChecked =
                    match inferProgram(
                        ProgramSyntax(items = [TopLevelTrait(
                            eqDeclaration
                        ), TopLevelImplementation(badArity)], body = None)
                    ) with
                ...
                in
                    let unorderedMethods =
                        TraitDecl(name = "UnorderedMethods", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [TraitMethodDecl(name = "zeta", signature = TypeArrow(
                            TypeNamed("a"),
                            TypeNamed("Bool"),
                            [],
                            None
                        ), defaultImplementation = None), TraitMethodDecl(name = "alpha", signature = TypeArrow(
                            TypeNamed("a"),
                            TypeNamed("Bool"),
                            [],
                            None
                    ...
                    in
                        let missingMethods =
                            TraitImplementationDecl(traitName = "UnorderedMethods", typeArguments = [TypeNamed(
                                "Int"
                            )], requirements = [], bindings = [])
                        in
                            match inferProgram(
                                ProgramSyntax(items = [TopLevelTrait(
                                    unorderedMethods
                                ), TopLevelImplementation(missingMethods)], body = None)
                            ) with
...
let rejectsExpandingTraitImplementationRequirement unit =
    (let expanding =
        TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
            "a"
        )], requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeApplied(
            "List",
            [TypeNamed("a")]
        )])], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(expanding)], body = None)
        ) with
...
let rejectsUnchangedTraitImplementationRequirement unit =
    ...
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(unchanged)], body = None)
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(NonDecreasingTraitImplementationRequirement("Equal", "Equal")) } -> Unit
...
let acceptsDecreasingTraitImplementationRequirement unit =
    ...
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(decreasing)], body = None)
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = None } -> Unit
...
let acceptsConcreteTraitImplementationRequirement unit =
    ...
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(concrete)], body = None)
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = None } -> Unit
...
let expectTraitConstraintCanonicalization unit =
    (let ranked =
        TraitDecl(name = "Ranked", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "Ordered", typeArguments = [TypeNamed(
            "a"
        )])], methods = [TraitMethodDecl(name = "rank", signature = TypeArrow(
            TypeNamed("a"),
            TypeNamed("Int"),
            [],
            None
    ...
    in
        let redundant =
            TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
                ...
                "Int"
            )]), TraitConstraintSyntax(traitName = "Ordered", typeArguments = [TypeNamed(
                "Int"
            )]), TraitConstraintSyntax(traitName = "Ranked", typeArguments = [TypeNamed(
                "Int"
            )]), TraitConstraintSyntax(traitName = "Ranked", typeArguments = [TypeNamed(
                "Int"
            )])], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
        in
            match inferProgram(
                ProgramSyntax(items = [TopLevelTrait(ranked), TopLevelTrait(
                    ordDeclaration
                ), TopLevelTrait(eqDeclaration), TopLevelImplementation(redundant)], body = None)
            ) with
                | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = environment, error = None } ->
                    match resolveTraitImplementations("Equal")(environment) with
                        ...
                        | _ ->
                            test.fail(
                                "implementation requirements should be deduplicated and omit transitively implied supertraits"
                            )
                | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                    test.fail(
                        "canonical implementation requirements should infer: " + Ashes.Trait.Show.show(error)
                    ))
...
let binaryTraitCall traitName methodName =
    ExprCall(
        ExprCall(ExprQualifiedVar(traitName)(methodName))(ExprVar("left"))(false)(callArgumentsInline),
        ExprVar("right"),
        false,
        callArgumentsInline
    )
...
let expectInferredConstraintCanonicalization unit =
    ...
    in
        ...
        in
            ...
            in
                match inferProgram(
                    ProgramSyntax(items = [TopLevelTrait(ordDeclaration), TopLevelTrait(eqDeclaration), TopLevelLet(
                        binding,
                        false
                    )], body = None)
                ) with
                    ...
                    | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                        test.fail(
                            "canonical inferred constraints should infer: " + Ashes.Trait.Show.show(error)
                        ))

let showDeclaration =
    TraitDecl(name = "Show", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [TraitMethodDecl(name = "show", signature = TypeArrow(
        TypeNamed("a"),
        TypeNamed("Str"),
        [],
        None
...
let sameValue =
    ExprLambda(
        "value",
        ExprCall(
            ExprCall(ExprQualifiedVar("Equal")("equal"))(ExprVar("value"))(false)(callArgumentsInline),
            ExprVar("value"),
            false,
            callArgumentsInline
        ),
...
let expectValidWrittenBindingRequirement unit =
    (let binding =
        LetBindingSyntax(name = "same", value = sameValue, sugarParameters = [], typeAnnotation = Some(
            sameAnnotation
        ), requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed("a")])])
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelLet(binding)(false)], body = None)
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = TypeEnvironment { bindings = bindings }, error = None } ->
            ...
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                test.fail(
                    "a justified written requires clause should infer: " + Ashes.Trait.Show.show(error)
                ))

let rejectsMissingWrittenBindingRequirement unit =
    (let binding =
        LetBindingSyntax(name = "bad", value = sameValue, sugarParameters = [], typeAnnotation = Some(
            sameAnnotation
        ), requirements = [TraitConstraintSyntax(traitName = "Show", typeArguments = [TypeNamed("a")])])
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelTrait(
                showDeclaration
            ), TopLevelLet(binding)(false)], body = None)
        ) with
...
let rejectsAmbiguousWrittenBindingRequirement unit =
    (let binding =
        LetBindingSyntax(name = "ambiguous", value = ExprInt(1), sugarParameters = [], typeAnnotation = Some(
            TypeNamed("Int")
        ), requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed("a")])])
    in
        match inferProgram(
...
let rejectsInvalidWrittenBindingRequirements unit =
    (let unknown =
        LetBindingSyntax(name = "unknown", value = ExprInt(1), sugarParameters = [], typeAnnotation = Some(
            TypeNamed("Int")
        ), requirements = [TraitConstraintSyntax(traitName = "Missing", typeArguments = [TypeNamed("Int")])])
    in
        let unknownChecked =
        ...
        in
            let wrongArity =
                LetBindingSyntax(name = "wrongArity", value = ExprInt(
                    1
                ), sugarParameters = [], typeAnnotation = Some(
                    TypeNamed("Int")
                ), requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [])])
            in
                match inferProgram(
                    ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelLet(wrongArity)(false)], body = None)
                ) with
...
let rejectsNestedMissingWrittenBindingRequirement unit =
    (let nested =
        ExprLet(
            ...
            [],
            Some(sameAnnotation),
            [TraitConstraintSyntax(traitName = "Show", typeArguments = [TypeNamed("a")])]
        )
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelTrait(showDeclaration)], body = Some(nested))
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(ProgramExpressionError(MissingWrittenTraitRequirement("Equal"))) } -> Unit
...
let acceptsRecursiveWrittenBindingRequirement unit =
    ...
    in
        match inferProgram(
            ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelLet(binding)(true)], body = None)
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = None } -> Unit
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
                test.fail(
                    "recursive written requires clauses should infer: " + Ashes.Trait.Show.show(error)
                ))

let rejectsInvalidRequirementInRecursiveGroup unit =
    (let valid =
        LetBindingSyntax(name = "first", value = sameValue, sugarParameters = [], typeAnnotation = Some(
            sameAnnotation
        ), requirements = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed("a")])])
    in
        let invalid =
            LetBindingSyntax(name = "second", value = sameValue, sugarParameters = [], typeAnnotation = Some(
                sameAnnotation
            ), requirements = [TraitConstraintSyntax(traitName = "Show", typeArguments = [TypeNamed("a")])])
        in
            match inferProgram(
...
let expectTraitImplementationBodyValidation unit =
    ...
    in
        let invalid =
            TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
                "Int"
            )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = invalidBody)])
        in
            match inferProgram(
                ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(invalid)], body = None)
            ) with
                | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(ProgramExpressionError(InferenceUnificationError(TypeMismatch(_left, _right)))) } -> Unit
...
let expectTraitImplementationCapabilityRows unit =
    ...
    in
        ...
        in
            let timed =
                TraitDecl(name = "Timed", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [TraitMethodDecl(name = "time", signature = TypeArrow(
                    TypeNamed("a"),
                    TypeNamed("Int"),
                    [("TraitClock", [])],
                    None
            ...
            in
                let clockBody =
                    ExprLambda(
                        "value",
                        ExprCall(ExprQualifiedVar("TraitClock")("now"))(ExprTuple([]))(false)(callArgumentsInline),
                        None
                    )
                in
                    let valid =
                        TraitImplementationDecl(traitName = "Timed", typeArguments = [TypeNamed(
                            "Int"
                        )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "time", implementation = clockBody)])
                    in
                        let validChecked =
                            match inferProgram(
                                ProgramSyntax(items = [TopLevelCapability(
                                    clock
                                ), TopLevelCapability(log), TopLevelTrait(
                                    timed
                        ...
                        in
                            let logBody =
                                ExprLambda(
                                    "value",
                                    ExprCall(
                                        ExprQualifiedVar("TraitLog")("write"),
                                        ExprTuple([]),
                                        false,
                                        callArgumentsInline
                                    ),
                            ...
                            in
                                let invalid =
                                    TraitImplementationDecl(traitName = "Timed", typeArguments = [TypeNamed(
                                        "Int"
                                    )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "time", implementation = logBody)])
                                in
                                    match inferProgram(
                                        ProgramSyntax(items = [TopLevelCapability(
                                            clock
                                        ...
                                        | _ ->
                                            test.fail(
                                                "implementation capability rows should match method signatures"
                                            ))
    ...
    in
        let exactChecked =
            match inferProgram(
                ProgramSyntax(items = [TopLevelTrait(
                    eqDeclaration
                ), TopLevelImplementation(exact), TopLevelImplementation(exact)], body = None)
            ) with
            ...
            in
                let genericFirstChecked =
                    match inferProgram(
                        ProgramSyntax(items = [TopLevelTrait(eqDeclaration), TopLevelImplementation(
                            genericEqualImplementation
                        ), TopLevelImplementation(concrete)], body = None)
                    ) with
                        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(OverlappingTraitImplementations("Equal")) } -> Unit
                ...
                in
                    let concreteFirstChecked =
                        match inferProgram(
                            ProgramSyntax(items = [TopLevelTrait(
                                eqDeclaration
                            ), TopLevelImplementation(concrete), TopLevelImplementation(
                                genericEqualImplementation
                            )], body = None)
                        ) with
                            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(OverlappingTraitImplementations("Equal")) } -> Unit
                    ...
                    in
                        let pairTrait =
                            TraitDecl(name = "PairEqual", typeParameters = [
                                TypeParameter(name = "a"),
                                TypeParameter(name = "b")
                            ], supertraits = [], methods = [TraitMethodDecl(name = "equal", signature = TypeArrow(
                                TypeNamed("a"),
                                TypeArrow(TypeNamed("b"))(TypeNamed("Bool"))([])(None),
                                [],
                                None
                            ), defaultImplementation = None)])
                        in
                            let repeated =
                                TraitImplementationDecl(traitName = "PairEqual", typeArguments = [TypeNamed(
                                    "a"
                                ), TypeNamed(
                                    "a"
                                )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
                            in
                                ...
                                in
                                    match inferProgram(
                                        ProgramSyntax(items = [TopLevelTrait(
                                            pairTrait
                                        ), TopLevelImplementation(repeated), TopLevelImplementation(
                                            distinct
                                        )], body = None)
                                    ) with
                                        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = environment, error = None } ->
                                            match resolveTraitImplementations("PairEqual")(environment) with
                                                ...
                                                | _ ->
                                                    test.fail(
                                                        "non-overlapping repeated-variable heads should both register"
                                                    )
                                        | _ ->
                                            test.fail(
                                                "a repeated head variable should reject incompatible concrete arguments"
                                            ))
...
let inferPackage packageId environment items =
    match inferProgramFromPackage(packageId)(environment)(ProgramSyntax(items = items, body = None)) with
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
            test.fail(
                "package should infer: " + Ashes.Trait.Show.show(error)
            )

let equalIntImplementation requirements =
    TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
        "Int"
    )], requirements = requirements, bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
...
let expectRecursiveTraitEvidence environment unit =
    match resolveTraitEvidence(
        TraitConstraint(traitName = "Equal", typeArguments = [SemList(SemInt)]),
        environment
    ) with
...
let expectAbstractTraitEvidence environment unit =
    match resolveTraitEvidence(
        TraitConstraint(traitName = "Equal", typeArguments = [SemList(SemVariable(99))]),
        environment
    ) with
...
let expectConcreteTraitEvidenceResolution unit =
    ...
    in
        let environment =
            inferPackage("traits")(emptyTypeEnvironmentForPackage("traits"))([TopLevelTrait(
                eqDeclaration
            ), TopLevelTrait(
                ordDeclaration
            ), []
            |> equalIntImplementation
            |> TopLevelImplementation, TopLevelImplementation(
                genericEqualImplementation
            ), TopLevelImplementation(orderedImplementation)])
        in
            unit
...
let expectMissingTraitEvidence environment unit =
    match resolveTraitEvidence(
        TraitConstraint(traitName = "Equal", typeArguments = [SemList(SemString)]),
        environment
    ) with
...
let expectTraitEvidenceDepthLimit environment unit =
    match resolveTraitEvidence(
        TraitConstraint(traitName = "Equal", typeArguments = [nestedListType(65)(SemInt)]),
        environment
    ) with
...
let expectAmbiguousTraitVariable environment unit =
    match requireTraitEvidence(
        TraitConstraint(traitName = "Equal", typeArguments = [SemVariable(99)]),
        environment
    ) with
    ...
    in
        ...
        in
            let cyclicEnvironment =
                inferPackage("traits")(emptyTypeEnvironmentForPackage("traits"))([TopLevelTrait(
                    eqDeclaration
                ), [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeApplied(
                    "List",
                    [TypeNamed("Int")]
                )])]
                |> equalIntImplementation
...
let ownedTypeImplementation unit =
    TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeNamed(
        "Card"
    )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = trueEqualImplementation)])
...
let rejectNestedLocalOrphan traitPackage unit =
    ...
    in
        match inferProgramFromPackage(
            "consumer",
            traitPackage,
            ProgramSyntax(items = [TopLevelType(cardType), TopLevelImplementation(implementation)], body = None)
        ) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(OrphanTraitImplementation("Equal")) } -> Unit
...
let expectOneOwnedHead traitPackage unit =
    (let implementation =
        TraitImplementationDecl(traitName = "PairEqual", typeArguments = [TypeNamed("Int"), TypeNamed(
            "Card"
        )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = pairEqualImplementation)])
    in
...
let expectTraitImplementationOrphanRule unit =
    (let traitPackage =
        inferPackage(
            "traits",
            emptyTypeEnvironmentForPackage("traits"),
            [TopLevelTrait(eqDeclaration), TopLevelTrait(pairEqualDeclaration)]
        )
    in
