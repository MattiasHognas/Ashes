...
| IR, optimizer, ownership, backend, linker | Complete IR model/text form plus core and scalar-control lowering for constants, lexical locals, strict curried calls, closures, captures, partial application, lifted functions, conditions, guarded scalar matches, and recursive groups | In progress; structural/builtin/external/evidence/async lowering, optimization, ownership, backend, and linking remain |
