...
let instructionTemps (kind: IrInstructionKind) =
    match kind with
        ...
        | MakeClosure(t1, _, t2, _, _, _, _, _) -> [t1, t2]
        | MakeClosureStack(t1, _, t2, _, _, _, _) -> [t1, t2]
        | LoadFuncAddr(t, _) -> [t]
        ...
        | AllocReusing(t1, _, _, t2, _, _, _) -> [t1, t2]
        | SetAdtField(t1, _, t2, _) -> [t1, t2]
        | SaveStackPointer(_) -> []
        | RestoreStackPointer(_) -> []
        | GetAdtTag(t1, t2) -> [t1, t2]
        | GetAdtField(t1, t2, _, _) -> [t1, t2]
        | PrintInt(t) -> [t]
        | PrintStr(t) -> [t]
        | PrintBool(t) -> [t]
        | WriteStr(t) -> [t]
