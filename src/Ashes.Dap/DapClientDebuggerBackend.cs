...
/// <summary>
/// Shared implementation for debugger backends that drive a native debugger
/// through its own Debug Adapter Protocol interpreter (<c>gdb
/// --interpreter=dap</c>, <c>lldb-dap</c>). The backend acts as a DAP client
/// to the debugger subprocess, translating <see cref="IDebuggerBackend"/>
/// operations into DAP requests over the child's stdio. Concrete backends
/// supply the process invocation and dialect details.
/// </summary>
public abstract partial class DapClientDebuggerBackend : IDebuggerBackend
{
    ...
    /// <summary>Guidance appended when the adapter binary cannot be started at all.</summary>
    protected abstract string InstallHint { get; }

    /// <summary>Builds the process invocation for the adapter.</summary>
    protected abstract ProcessStartInfo CreateAdapterStartInfo(string? debuggerPath);

    /// <summary>Builds the DAP launch request arguments in the adapter's dialect.</summary>
    protected abstract object CreateLaunchArguments(string program, string? cwd, string[]? args, bool stopOnEntry);

    /// <summary>
    /// Whether variables in the named scope belong in the DAP locals response. GDB splits
    /// function arguments into a separate "Arguments" scope; LLDB folds them into "Locals".
    /// </summary>
    protected virtual bool IsVariablesScope(string? scopeName)
    {
    ...
    public async Task StartAsync(string program, string? cwd, string[]? args, string? debuggerPath, bool stopOnEntry)
    {
        ...
        // Debug adapters answer the launch request only after configurationDone,
        // so the response is awaited later in RunAsync.
        _launchResponse = SendRequestWithoutAwaiting("launch", CreateLaunchArguments(program, cwd, args, stopOnEntry));
    ...
    public async Task<DapVariable[]> GetLocalsAsync()
    {
        ...
        foreach (var scopeReference in scopeReferences)
        {
            var body = await SendRequestAsync("variables", new { variablesReference = scopeReference }).ConfigureAwait(false);
    ...
    private async Task<(List<int> ScopeReferences, int FrameId)> GetVariableScopeReferencesAsync()
    {
        ...
        {
            return (references, frameId);
        }

        foreach (var scope in scopes.EnumerateArray())
        {
            if (IsVariablesScope(GetStringProperty(scope, "name"))
                && scope.TryGetProperty("variablesReference", out var reference))
            {
                references.Add(reference.GetInt32());
            }
        }
    ...
    private async Task<string?> EvaluateExpressionAsync(string expression, int frameId)
    {
        try
        {
            ...
            // "(type) $N = ..." and (in lldb-dap) mis-evaluates cast expressions.
    ...
    /// <summary>
    /// Debugger expression results may look like <c>(long) $0 = 42</c>; strip the
    ...
    private TaskCompletionSource<JsonElement> SendRequestWithoutAwaiting(string command, object arguments)
    {
        ...
        {
            throw new InvalidOperationException($"{AdapterDisplayName} is not running.");
        }
    ...
    private async Task<JsonElement> AwaitResponseAsync(TaskCompletionSource<JsonElement> tcs, string command)
    {
        ...
        using var registration = cts.Token.Register(
            () => tcs.TrySetException(new TimeoutException($"Timed out waiting for {AdapterDisplayName} response to '{command}'.")));

        var response = await tcs.Task.ConfigureAwait(false);

        if (response.TryGetProperty("success", out var success) && !success.GetBoolean())
        {
            var message = GetStringProperty(response, "message") ?? $"{AdapterDisplayName} rejected '{command}'.";
    ...
    private async Task WaitForInitializedEventAsync()
    {
        using var cts = new CancellationTokenSource(RequestTimeout);
        using var registration = cts.Token.Register(
            () => _initializedEvent.TrySetException(new TimeoutException($"Timed out waiting for the {AdapterDisplayName} initialized event.")));
    ...
    private async Task<InvalidOperationException> CreateStartupFailureAsync(ProcessStartInfo startInfo, Process process)
    {
        ...
        var message = $"{AdapterDisplayName} exited immediately when started as '{BuildCommandDisplay(startInfo)}'.";
    ...
    private InvalidOperationException CreateStartFailure(string adapterPath, Exception? innerException)
    {
        var message = $"Failed to start {AdapterDisplayName} using '{adapterPath}'. {InstallHint}";
    ...
    private InvalidOperationException CreateCommandFailure(string command, Exception innerException)
    {
        if (_adapter is not null && _adapter.HasExited)
        {
            var message = $"{AdapterDisplayName} exited before handling '{command}' (exit code {_adapter.ExitCode.ToString(CultureInfo.InvariantCulture)}).";
            if (!string.IsNullOrWhiteSpace(_launchError))
            {
                message += $" {NormalizeDiagnostic(_launchError)}";
            }
        ...
        return new InvalidOperationException($"Failed to send request to {AdapterDisplayName}: {command}", innerException);
    }
