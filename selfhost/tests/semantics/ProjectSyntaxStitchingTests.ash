...
let coreUnit =
    SemanticStitchUnit(name = "Core", packageId = "dep-core@1.0.0", sourcePath = "/deps/Core.ash", imports = [], interface = moduleInterface(
        "Core",
        "seed"
    ), program = ProgramSyntax(items = [TopLevelExport(
        ExportDecl(items = [ExportValue("seed")])
    ), false
    |> TopLevelLet(binding("seed")(ExprInt(1)))
    |> TopLevelAt(TextSpan(start = 10, end = 20))], body = Some(ExprInt(999))), isEntry = false)

let utilUnit =
    SemanticStitchUnit(name = "Util", packageId = "dep-util@2.0.0", sourcePath = "/deps/Util.ash", imports = [ResolvedModuleImport(
        "Core",
        None,
        1,
        "import Core"
    )], interface = moduleInterface("Util")("incremented"), program = ProgramSyntax(items = [TopLevelExport(
        ExportDecl(items = [ExportValue("incremented")])
    ), false
...
let mainUnit =
    SemanticStitchUnit(name = "Main", packageId = "app@1.0.0", sourcePath = "/app/Main.ash", imports = [ResolvedModuleImport(
        ...
        1,
        "import Util"
    )], interface = moduleInterface("Main")("result"), program = ProgramSyntax(items = [TopLevelExport(
        ExportDecl(items = [ExportValue("result")])
    ), false
...
let expectCombinedProgram project =
    match project with
        ...
        | _ ->
            test.fail(
                "combined syntax should retain dependency order, rewritten references, spans, and only the entry body"
            )
...
let expectModuleRegions project =
    match project with
        | StitchedSyntaxProject { moduleRegions = regions } ->
            ...
            |> test.assertEqual(
                [
                    StitchedModuleRegion(moduleName = "Core", packageId = "dep-core@1.0.0", sourcePath = "/deps/Core.ash", itemStart = 0, itemEnd = 1, isEntry = false),
                    StitchedModuleRegion(moduleName = "Util", packageId = "dep-util@2.0.0", sourcePath = "/deps/Util.ash", itemStart = 1, itemEnd = 2, isEntry = false),
                    StitchedModuleRegion(moduleName = "Main", packageId = "app@1.0.0", sourcePath = "/app/Main.ash", itemStart = 2, itemEnd = 3, isEntry = true)
                ]
            )
            |> (given (_) -> project)
