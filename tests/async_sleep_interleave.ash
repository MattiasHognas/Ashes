...
// Ashes.Task.all the scheduler steps both to their first suspend (A1, then B1), then waits only until
...
let taskA =
    ...
    in
        match await Ashes.Task.sleep(50) with
            | Error(_) -> 0
            | Ok(_) ->
...
let taskB =
    ...
    in
        match await Ashes.Task.sleep(10) with
            | Error(_) -> 0
            | Ok(_) ->
