// expect: 5 40
// CO-2c: admit a Map.set RESULT as a move seed. `Ashes.Collection.Map.set(k)(v)(m)` returns its accumulator `m`,
// rebuilt/rebalanced in place on the reuse path, so seeding a reuse fold from `Ashes.Collection.Map.set(...)(m)` —
...
//
// `outer` threads a growing map, re-seeding the inner reuse fold `inner` from `Ashes.Collection.Map.set(...)(m)`
...
let recursive inner i lim m =
    ...
    then m
        ...
        m
        |> Ashes.Collection.Map.setWith(cmp)(i)(i * 10)
        |> inner(i + 1)(lim)
...
let recursive outer b nb m =
    ...
    then m
        ...
        m
        |> Ashes.Collection.Map.setWith(cmp)(0)(0)
        |> inner(0)(4)
        |> outer(b + 1)(nb)
...
in
    match Ashes.Collection.Map.getWith(cmp)(4)(final) with
        ...
        | Some(v) ->
            Ashes.IO.print(Ashes.Text.fromInt(Ashes.Collection.Map.size(final)) + " " + Ashes.Text.fromInt(v))
