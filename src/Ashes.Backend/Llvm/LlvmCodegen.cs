...
internal static partial class LlvmCodegen
{
    ...
    private static bool? EmitInstructionGroup6(LlvmCodegenState state, IrInst instruction, int index, IReadOnlyList<IrInst> instructions)
    {
        return instruction switch
        {
            ...
            IrInst.MakeClosure makeClosure => StoreTemp(state, makeClosure.Target, EmitMakeClosure(
                ...
                makeClosure.AcceptsRuntimeManagedArgument, makeClosure.ReturnsGeneralRcOwned)),
            ...
            IrInst.MakeClosureStack makeClosureStack => StoreTemp(state, makeClosureStack.Target,
                EmitMakeClosureStack(state, makeClosureStack.FuncLabel, LoadTemp(state, makeClosureStack.EnvPtrTemp),
                    makeClosureStack.EnvSizeBytes, makeClosureStack.ReturnsRuntimeManaged, makeClosureStack.AcceptsRuntimeManagedArgument, makeClosureStack.ReturnsGeneralRcOwned)),
    ...
    private static bool? EmitInstructionGroup7(LlvmCodegenState state, IrInst instruction, int index)
    {
        return instruction switch
        {
            IrInst.AllocFfiOut allocateOut => StoreTemp(
                state,
                allocateOut.Target,
                EmitAllocFfiOut(state, allocateOut.ElementType)),
            IrInst.LoadFfiOut loadOut => StoreTemp(
                state,
                loadOut.Target,
            ...
            IrInst.CopyFfiBytes copyBytes => StoreTemp(
                state,
                copyBytes.Target,
            ...
            IrInst.GetAdtField getAdtField => StoreTemp(state, getAdtField.Target, LoadAdtField(state, LoadTemp(state, getAdtField.Ptr), getAdtField.FieldIndex, $"get_adt_field_{getAdtField.Target}", getAdtField.Tagless)),
