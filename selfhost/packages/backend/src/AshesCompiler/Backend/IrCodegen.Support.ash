...
// scalar-type bundle, raw linux-x64 syscall wrappers (inline `syscall` assembly — `rcx`/`r11`
// clobbered, a fourth argument rides `r10`), the view-aware `Str`/`Bytes` header readers and
// writers, RC-managed allocation with the real 16-byte `{count, size}` header, closure-object
// layout and calls, `RcDrop`, string concatenation/printing/comparison, and the `PrintInt`/
// `PrintBool` digit machinery. Everything here depends only on the LLVM bindings — the
// per-instruction dispatch and the builtin-family emitters live in the sibling slices
// (`IrCodegen.Filesystem`, `IrCodegen.TextBytes`) and in `IrCodegen` itself.
