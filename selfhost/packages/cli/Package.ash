...
// - Only `add`, `fmt`, `init`, `remove`, `tree`, and `why` are implemented so far; the other
//   subcommands remain unported (see docs/md/future/SELF_HOSTING.md's CLI checklist). Dispatch
//   itself lives in `AshesCompiler.Cli.Dispatch` (`runCli`), not here, so it stays directly
//   testable the same way every other command is.
