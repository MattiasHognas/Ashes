...
| IR, optimizer, ownership, backend, linker | Complete IR model/text form plus core expression lowering for constants, lexical locals, strict curried calls, closures, captures, partial application, and lifted functions | In progress; control flow, structural/builtin/external/evidence/async lowering, optimization, ownership, backend, and linking remain |
