...
let expectDirectCapabilityEffects unit =
    (let clockCall =
        ExprCall(ExprQualifiedVar("Clock")("now"))(ExprTuple([]))(false)
    in
        let logCall =
            ExprCall(ExprQualifiedVar("Log")("write"))(ExprString("message"))(false)
        in
...
let expectHigherOrderEffects unit =
    (let higherOrder =
        ExprLambda("operation")(ExprCall(ExprVar("operation"))(ExprTuple([]))(false))(None)
    in
        match Unit
...
let expectInferredLogEffect unit =
    (let lambda =
        ExprLambda("ignored")(ExprCall(ExprQualifiedVar("Log")("write"))(ExprString("message"))(false))(None)
...
let expectResultMapperEffect unit =
    ...
    in
        ...
        in
            ...
            in
                match inferExpression(ExprLambda("ignored")(ExprResultPipe(ExprVar("input"))(ExprVar("mapper")))(None))(environment) with
                    | TypeInferenceResult { semanticType = semanticType, substitution = substitution, supply = _supply, constraints = _constraints, error = None } ->
                        match applySubstitution(substitution)(semanticType) with
                            | SemFunction(_argument, _result, Some(row)) ->
...
let expectInvalidPerform unit =
    ...
    in
        let missingCall =
            false
            |> ExprCall(ExprQualifiedVar("Clock")("missing"))(ExprTuple([]))
            |> ExprPerform
        in
    ...
    in
        ...
        in
            ...
            in
                ...
                in
                    let loggingArmBody =
                        ExprLet("ignored")(ExprCall(ExprQualifiedVar("Log")("write"))(ExprString("handled"))(false))(resumeWith(ExprInt(42)))([])(None)([])
                    in
                        ...
                        in
                            let armEffectChecked =
                                [loggingArm]
                                |> ExprHandle(handlerClockCall)
                                |> expectHandlerDischarges("Int")("Clock")(Some("Log"))([TopLevelCapability(clockDeclaration), TopLevelCapability(logDeclaration)])
                            in
                                let stateGet =
                                    ExprCall(ExprQualifiedVar("State")("get"))(ExprTuple([]))(false)
                                in
                                    ...
                                    in
                                        ...
                                        in
                                            let stateChecked =
                                                [getArm, setArm]
                                                |> ExprHandle(stateGet)
                                                |> expectHandlerDischarges("Int")("State")(None)([TopLevelCapability(stateDeclaration)])
                                            in Unit)
        ...
        in
            ...
            in
                let incompleteChecked =
                    ...
                    |> ExprHandle(ExprCall(ExprQualifiedVar("State")("get"))(ExprTuple([]))(false))
                    |> Some)) with
                        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(ProgramExpressionError(InvalidHandler(_message))) } -> Unit
                ...
                in
                    ...
                    in
                        let patternChecked =
                            match inferProgram(ProgramSyntax(items = [TopLevelCapability(clockDeclaration)], body = [badPatternArm]
                            |> ExprHandle(handlerClockCall)
                            |> Some)) with
                                | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(ProgramExpressionError(InferenceUnificationError(TypeMismatch(_left, _right)))) } -> Unit
                        ...
                        in
                            ...
                            in
                                let resumeChecked =
                                    match inferProgram(ProgramSyntax(items = [TopLevelCapability(clockDeclaration)], body = [badResumeArm]
                                    |> ExprHandle(handlerClockCall)
                                    |> Some)) with
                                        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(ProgramExpressionError(InferenceUnificationError(TypeMismatch(_left, _right)))) } -> Unit
...
let expectProviderSatisfaction unit =
    ...
    in
        ...
        in
            let stateSet =
                ExprCall(ExprQualifiedVar("State")("set"))(ExprInt(42))(false)
            in
                ...
                in
                    ...
                    in
                        let abstractSet =
                            ExprLambda("value")(ExprCall(ExprQualifiedVar("State")("set"))(ExprVar("value"))(false))(None)
                        in
...
let rejectMissingProviderOperation unit =
    (let provider = ProvideDecl(capabilityName = "State", typeArguments = [TypeNamed("Int")], bindings = [ProvideBinding(operationName = "get", implementation = ExprLambda("ignored")(ExprInt(0))(None))])
    in
        match inferProgram(ProgramSyntax(items = [TopLevelCapability(stateDeclaration), TopLevelProvide(provider)], body = None)) with
...
let rejectUnknownProviderOperation unit =
    (let provider = ProvideDecl(capabilityName = "Clock", typeArguments = [], bindings = [ProvideBinding(operationName = "missing", implementation = ExprLambda("ignored")(ExprInt(0))(None))])
    in
        match inferProgram(ProgramSyntax(items = [TopLevelCapability(clockDeclaration), TopLevelProvide(provider)], body = None)) with
...
let rejectDuplicateProviderOperation unit =
    (let provider = ProvideDecl(capabilityName = "Clock", typeArguments = [], bindings = [ProvideBinding(operationName = "now", implementation = ExprLambda("ignored")(ExprInt(0))(None)), ProvideBinding(operationName = "now", implementation = ExprLambda("ignored")(ExprInt(1))(None))])
    in
        match inferProgram(ProgramSyntax(items = [TopLevelCapability(clockDeclaration), TopLevelProvide(provider)], body = None)) with
...
let rejectProviderCapabilityArity unit =
    match inferProgram(ProgramSyntax(items = [TopLevelCapability(stateDeclaration), TopLevelProvide(ProvideDecl(capabilityName = "State", typeArguments = [], bindings = []))], body = None)) with
...
let rejectProviderImplementationType unit =
    (let provider = ProvideDecl(capabilityName = "Clock", typeArguments = [], bindings = [ProvideBinding(operationName = "now", implementation = ExprLambda("ignored")(ExprString("wrong"))(None))])
    in
        match inferProgram(ProgramSyntax(items = [TopLevelCapability(clockDeclaration), TopLevelProvide(provider)], body = None)) with
            | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(ProgramExpressionError(InferenceUnificationError(TypeMismatch(_left, _right)))) } -> Unit
