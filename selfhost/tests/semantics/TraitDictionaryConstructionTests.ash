...
let unaryStringMethod name defaultImplementation =
...
let displayIntImplementation unit =
    TraitImplementationDecl(traitName = "Display", typeArguments = [TypeNamed(
        "Int"
    )], requirements = [], bindings = [TraitImplementationMethodBinding(methodName = "tag", implementation = ExprLambda(
        "value",
        ExprString("tag"),
        None
    ))])
...
let displayEnvironment unit =
    ...
    |> TopLevelImplementation], body = None)) with
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
            test.fail(
                "display implementation should infer: " + Ashes.Trait.Show.show(error)
            )
...
let genericEqualImplementation unit =
    TraitImplementationDecl(traitName = "Equal", typeArguments = [TypeApplied(
        ...
        "a"
    )])], bindings = [TraitImplementationMethodBinding(methodName = "equal", implementation = ExprLambda(
        "left",
...
let conditionalEvidenceEnvironment unit =
    ...
    |> TopLevelImplementation], body = None)) with
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } ->
            test.fail(
                "conditional Equal implementation should infer: " + Ashes.Trait.Show.show(error)
            )
...
let planResolvedDictionary constraint environment =
    match resolveTraitEvidence(constraint)(environment) with
        | TraitEvidenceResolution { plan = Some(evidence), error = None } ->
            planTraitDictionaryConstruction(
                evidence,
                environment
            )
