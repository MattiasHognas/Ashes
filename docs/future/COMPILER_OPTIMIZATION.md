...
| **CO-12** | **Zero-copy borrowed slices from `subText`/`substring`** | The `mmap`-input half **landed** (`Ashes.File.mmap`, see *Completed Work*) — the string/bytes *view* representation already exists (`EmitStringView`). What remains: make `Bytes.subText`/`String.substring` return a **view** into their source rather than copying, so field extraction is allocation-free. **Caveat found:** it is not a clear win — a slice that is immediately stored in a structure (e.g. a 1BRC station name inserted into the `Map`) is *materialized* (copied) into the persistent blob on insert anyway, so a view is created then copied — strictly more work than a direct copy. It only helps **transient** slices (parsed/compared then discarded). So a useful version must be context-sensitive (view when the slice doesn't escape, copy when it's stored) — which needs escape/lifetime analysis. Endpoint: a field-slicing fold that keeps only transient slices shows an allocation/time drop with no correctness regression. Risk: high (slice lifetime vs. backing; interacts with the reuse materialization). |
| **CO-14** | **Data-parallel chunked fold (mergeable parallel reduce over a byte range / file)** | A streaming fold like 1BRC's `streamLoop` is single-threaded; the goal is to split a large input into per-core chunks, fold each on a worker, and merge. **Mechanism proven + tested — it already works via the existing `Ashes.Parallel.reduce` with no compiler change:** represent chunks as a list of `(bytes, lo, hi)` tuples and call `Ashes.Parallel.reduce(merge)(emptyAcc)(foldChunk)(chunks)`, where `foldChunk` folds one byte range into a partial `Map` and `merge` combines two `Map`s. The monomorphized `reduce` forks via `both` at the concrete `Map` result type, workers read the shared read-only `Bytes`, and the partial `Map`s are deep-copied across the fork and merged. Verified: correct + deterministic on linux-x64, linux-arm64 (qemu), win-x64 (Wine) (`tests/parallel_chunked_fold.ash`), and **7.65× faster** on an 8-chunk CPU-bound microbench (same total work, 435 ms → 57 ms; worker cap is 8). **Sharp edges found (must observe):** `foldChunk` must be a top-level, **non-capturing** function — thread `bytes` through the chunk tuple, not a closure capture (an arena closure crossing a fork is unsafe); a worker that reads out of bounds (`Bytes.get`) deadlocks the join; `Ashes.File.readText` caps at 1 MiB (~64k rows), so a real >1 MiB 1BRC needs an uncapped whole-file read; Ashes has no `\|\|`/`&&` (use nested `if`); whole-module `import` of two modules that share a name (e.g. `Ashes.List` and `Ashes.Parallel` both export `map`) collides. **Follow-up landed — real full-file parallel 1BRC:** added `Ashes.File.readAllBytes` (whole file into a `Bytes`, no UTF-8 validation; uncapped on Linux via a standalone `mmap`, since a single arena allocation can't exceed one chunk — Windows still shares the capped `readText` buffer). With it, `challenges/1brc/brc_parallel.ash` reads the whole file, splits it into 8 newline-aligned chunks, folds each into a partial station `Map` on a worker, and merges — **byte-identical to the sequential `brc` at 1M and 10M**, and **5.07× faster at 10M rows (13.26 s → 2.62 s)**. Memory is the tradeoff: ~13 GB (whole file in memory + 8 per-worker non-reuse `Map`s + deep-copy merge) vs the sequential fold's 50 MB — a speed/memory tradeoff the caller opts into. Regressions: `tests/parallel_chunked_fold.ash`, `tests/file_read_all_bytes.ash`. **Still remaining:** an ergonomic `Ashes.Parallel.reduceChunks` combinator — **blocked** by the no-inter-file-import stdlib rule (`Ashes.Parallel` can't call `Ashes.Bytes` for record-boundary splitting), so it stays a user-level pattern (as in `brc_parallel.ash`) until that rule is lifted. Risk: med (landed). |
...
speedup and its mechanism is now proven + tested (7.65× on 8 chunks) — the remaining work there is
ergonomics + an uncapped file read; **CO-12**'s zero-copy `mmap` input has landed (`Ashes.File.mmap`),
leaving borrowed `subText`/`substring` slices as the deeper single-core constant-factor work.
