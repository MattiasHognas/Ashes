...
Running the full Benchmarks Game suite in its **natural spelling** surfaced 17 real defects and
gaps. **All but one are fixed.** Fixed entries are cleared from this file once shipped — the
analysis and measurements live in [`docs/md/internals/changelog.md`](../docs/md/internals/changelog.md)
(rows CO-28 through CO-36 cover the memory-model arc), each fix ships with a regression test under
`tests/`, and the per-challenge `README.md` files carry the final benchmark numbers.
