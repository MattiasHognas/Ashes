...
runtime-managed representation even when the frame can never fully arena-reset; see "Phase 6,
profitability-gated veto relaxation" below. The four-classifier structural collapse remains unstarted.
Phase 7 is not yet started.
...
being cleared along with it. The full structural collapse of classifiers A-D into one per-parameter
decision remains unstarted and is where the bulk of the remaining risk and effort live. Phase 7 is deliberately last: it requires new test infrastructure that doesn't exist yet, touches the
