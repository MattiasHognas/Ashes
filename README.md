...
| **Ashes.Semantics** | Binding, scope resolution, type inference, ownership analysis, IR lowering |
| **Ashes.Backend** | LLVM code generation and native linking |
...
Ashes uses compiler-inferred RC Perceus for escaping ordinary heap graphs:
ownership moves avoid unnecessary increments, sharing inserts `dup`, and the
...
**in place** when a dead matched cell is uniquely owned. A Perceus reuse token
uses that cell directly; a shared cell is decremented and the constructor falls
back to a fresh allocation. This is representation-level mutation generated by
the compiler, not a mutable source value: the standard library remains pure and
the original value's semantics never change.
*Details:
[Drop specialization and reuse](docs/md/internals/architecture.md#drop-specialization-and-reuse)
...
`Ashes.Task.Parallel` workers run on independently owned heaps; captures and
results are deep-copied across the boundary, so non-atomic RC cells never alias
between threads.
*Details: [Evaluation Strategy](docs/md/reference/language.md#15-evaluation-strategy)
in the spec;
[Threads and structured parallelism](docs/md/internals/architecture.md#threads-and-structured-parallelism)
...
no ordinary-value use-after-move errors. The compiler emits the necessary
runtime RC operations and keeps resource-affinity diagnostics separate.
*Details: [Ownership Model](docs/md/reference/language.md#17-ownership-model).*

**Where do async tasks allocate?**
A `Task` is a small heap state struct in a scheduler-owned region, holding
...
stack frame. Scoped tasks return through scheduler watermarks; a spawned root's
private region is reaped on completion. Tasks execute single-threaded on the
caller thread.
