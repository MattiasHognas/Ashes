...
| Backend | LLVM C API bindings only so far (`selfhost/packages/backend`, `AshesCompiler.Backend.Llvm`): context/module/builder/type/value/basic-block creation, `functionType`, `addFunction`, `appendBasicBlock`, `constInt`, `buildRet`, and `verifyModule`, covering just enough of `src/Ashes.Backend/Llvm/Interop/LlvmApi.cs`'s surface to build and verify a trivial function in a real LLVM module end to end — verified by actually running the compiled test binary with a `libLLVM` build placed next to it, not just compiling it. IR-to-LLVM codegen itself, target/ABI/linking, and the rest of the LLVM C API surface are unported |
