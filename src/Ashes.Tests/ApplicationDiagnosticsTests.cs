...
public sealed class ApplicationDiagnosticsTests
{
    ...
    {
        var diag = LowerExpression("Ashes.IO.File.nope(\"hello\")", importAshesIO: false);

        diag.Errors.ShouldContain(x => x.Contains("Unknown member 'nope' in module Ashes.IO.File.", StringComparison.Ordinal));
        diag.Errors.ShouldNotContain(x => x.Contains("Unknown module 'Ashes.IO.File'.", StringComparison.Ordinal));
    }
