// A loop with two list accumulators, one consed onto only when a callee's pair carries a value:
// the pair holds a reference-counted record beside a Maybe, so it is placed on the
// reference-counted heap with both arms' elements, the conditional accumulator is placed there
// too, and the caller releases everything the loop returns.
// expect: 882909
type Tok =
    | kind: Int
    | text: Str
