// A bespoke 1BRC solution: the full 1e9-row challenge in ~8.3 s on a 32-thread desktop, with
// output byte-identical to a straightforward sequential fold. Shape: mmap the whole file
// (zero-copy), split it into per-core chunks at newline boundaries, fold each chunk on a worker
// thread, and merge the partial tables. Purity makes the fold order-independent, so the merged
// output is deterministic regardless of how chunks and workers are scheduled.
//
// The per-worker table is a purpose-built 16-ary hash trie whose leaf stores the min/max/sum/count
...
// aggregates" that fast 1BRC entries use, expressed in Ashes, kept in-place at constant memory per
// worker by the compiler's reuse specialization.
//
// The hot path trusts the 64-bit FNV-1a hash: an equal hash is treated as an equal key, with no
// leaf key byte-compare on hit. That removes one cold, scattered mmap read per row (the stored key
// view points at a station name's first occurrence, far from the streaming cursor) — the dominant
