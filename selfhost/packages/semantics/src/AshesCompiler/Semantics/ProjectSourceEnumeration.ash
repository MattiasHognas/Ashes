...
let recursive entryTasks style directory names =
    match names with
        | [] -> []
        | name :: rest ->
            SourceEntry(join(style)(deepCopy(directory))(name)) :: entryTasks(style)(directory)(rest)

let recursive rootTasks style roots =
    match roots with
        | [] -> []
        | root :: rest ->
            SourceRoot(normalize(style)(root)) :: rootTasks(style)(rest)
    ...
    match tasks with
        ...
        | SourceRoot(root) :: rest ->
            ...
            |> Ashes.IO.Directory.entries with
                | Error(error) ->
                    error
                    |> ProjectSourceRootReadError(root)
                    |> Error
                | Ok(names) ->
                    enumerateTasks(style)(appendList(entryTasks(style)(root)(names))(rest))(reversed)
        | SourceEntry(path) :: rest ->
            ...
            then enumerateTasks(style)(rest)(path :: reversed)
                ...
                |> Ashes.IO.Directory.entries with
                    ...
                    | Ok(names) ->
                        enumerateTasks(style)(appendList(entryTasks(style)(path)(names))(rest))(reversed)
...
let enumerateProjectSourceFiles style roots =
    match enumerateTasks(style)(rootTasks(style)(roots))([]) with
        ...
        | Ok(reversed) ->
            []
            |> uniquePaths(reverseList(reversed))
            |> Ok
