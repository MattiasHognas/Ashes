...
| `s1rawprog.sh <program-name> [n]` | After `s1diffprog.sh`, the same comparison with temp, local and label numbers kept: what the byte-for-byte parity tests will see. |
| `s0harness.sh [fixture]...` | Stage 0 against itself: each shared fixture's stored IR (lowered by the C# parity test from the bare parsed program, with no trait declarations registered) against the compiler's own `--emit-ir lowered` of the same source. A fixture listed here is one where a stage-1 difference may be the fixture's, not stage 1's. |
| `s1rawside.sh <program-name> <from> <to>` | The two raw dumps side by side over a line range: where a numbering difference starts, and which allocation one stage makes that the other does not. |
...
| `s1dumpat.sh <commit> <source> <out>` | Stage 1's lowered IR of a source file as the lowering stood at a commit. With `normdiff.sh <a> <b>` this makes good-against-bad stage 1 a two-minute check. It removes the dump tool it built, which holds that commit's lowering: rebuild with `-b` before the next comparison. |
...
| `parseloop.sh [-b] <source.ash> <rounds>...` | The real self-hosted lexer and parser run N times over one source with its header stripped (`projects/parseloop`). One parse of `Parser.ash` leaves 180 MB behind: each declaration value is parsed from a temporary copy of its tokens, and a tuple returning a list parameter holds that copy in the arena where nothing releases it (`reproducers/list_parameter_returned_inside_tuple.ash`). |
