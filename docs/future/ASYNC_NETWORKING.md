...
| **Async-only enforcement** | Enforced by the `Task` type, not a dedicated diagnostic: networking builtins return `Task(Str, …)`, which must be consumed via `await` / `Ashes.Async.run`, so misuse surfaces as an ordinary type error. (An earlier `ASH012` "outside `async`" code was reserved for an `async`-block model the language no longer has; it was never emitted and has been retired.) |
...
| **Windows readiness TCP stepping** | The Windows TCP connect/send/receive leaf-step helpers now preserve per-task progress, switch sockets into non-blocking mode, and resume through `WSAPoll` readiness waits so the same pending-wait model now also drives the landed TLS handshake and encrypted send/receive stages. |
| **Platform wait scaffolding** | The backend now carries the Linux syscall and Windows import/link scaffolding needed for task waits (`epoll` syscalls on Linux; Winsock readiness waits and PE linker entries on Windows). |
| **Shared combinator scheduler** | `Ashes.Async.all` and `Ashes.Async.race` now step tasks incrementally through shared runtime helpers that can drive arbitrary tasks until they are pending or complete, wait on any registered pending task in the list, and prefer the task whose ready wait handle actually woke the shared wait before continuing the scan. |
