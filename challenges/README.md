...
| [fannkuch-redux](fannkuch-redux/README.md) | N=11 | 37.3 s | 13.4 GB | correct `556355 / 51` output at every N, but peak RSS scales with `N!` (not constant): N=9 99 MB, N=10 1.1 GB, N=11 13.4 GB. The flat-memory state this row used to describe was superseded by the pattern-binding Perceus cutover (`docs/md/internals/changelog.md`'s "Pattern-binding Perceus cutover" entry, PR #357) and knowingly deferred to a future memory-model milestone — see the `.ash` file's own header comment. Not a regression from this repo's more recent work: bisected to PR #357, which predates it. |
