...
- **(result-alias / higher-order seed)** a saturated **call to a registered function** admitted by the
  ...
  *construction* to a builder *call*). The summary (`ComputeResultReach`, a monotone **least** fixpoint
  ...
  makes **internal sharing** poison: a parameter reachable through two simultaneous heap positions (e.g.
  `Node(x)(0)(x)`, or a `match` arm rebuilding with two sub-values of the same scrutinee) hits the cap and
  poisons, because moving such an argument would leave two live aliases the reuse fold could corrupt. The
...
(constant memory) — the specialization-path copy runs once, not per re-entry. All twelve
...
let-bound `wrap(7)(…)` result is **declined** (`keep` reads 7, not the corrupted 207). The
`grow/t elide=True` / `bump/t elide=False` split was confirmed at the direct-reuse elide decision.
