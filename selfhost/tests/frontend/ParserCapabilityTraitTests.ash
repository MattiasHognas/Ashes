...
let run unit =
    (let declarationsChecked = checkDeclarations(Unit)
    in
        let literalBindingsChecked = checkLiteralProviderBindings(Unit)
        in
            let emptyCapabilityChecked = expectDiagnostics("capability Empty =\n0")
            in
                let emptyProviderChecked = expectDiagnostics("provide Empty =\n0")
                in
                    let emptyTraitChecked = expectDiagnostics("trait Empty =\n0")
                    in expectDiagnostics("implement Empty(Int) =\n0"))
