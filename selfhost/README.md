...
| Backend | LLVM C API bindings only so far (`selfhost/packages/backend`, `AshesCompiler.Backend.Llvm`) — a growing subset covering context/module/builder/type/value/basic-block/global creation, arithmetic and comparison, control flow with the no-`phi` alloca/store/load slot pattern, function calls, and object/assembly emission with host-CPU-tuned target machines. `selfhost/tests/backend` proves each addition end to end by building, verifying, and emitting a small real function to genuine linux-x64 ELF objects and assembly listings, checked with `readelf` and independent exact-instruction assembly dumps (not just compiling or a byte-length assertion) — see `docs/md/future/SELF_HOSTING.md`'s "LLVM code generation and runtime integration" checklist for the current bound-function list and exact examples. No optimization-level selection yet, and IR-to-LLVM codegen itself, linking, the other three target RIDs, and the rest of the LLVM C API surface are unported |
