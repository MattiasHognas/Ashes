...
- [~] Expand `deriving {Eq, Ord, Show, Hash}` into ordinary implementations before coherence checking.
  ...
  recursive fields are rejected; resource, opaque-external, capability, and handler rejection remains
  coupled to the pending whole-program external/module type environment.
