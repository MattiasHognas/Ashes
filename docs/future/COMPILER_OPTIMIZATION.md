...
1. A non-blocking async scheduler / event loop and non-blocking networking leaf tasks (the enabling
   ...
   **CO-7a-2** (cooperative scheduler + timer queue) and **CO-7b-1/2/3** (per-target non-blocking
   networking leaf tasks). **CO-7a-2 DONE** — cooperative `sleep` suspension (`WaitKind = WaitTimer`,
   earliest-deadline wait + decrement) lands the timer half; two tasks now interleave across sleeps.
   The networking leaf tasks (CO-7b-*) are the remaining, per-target half.
