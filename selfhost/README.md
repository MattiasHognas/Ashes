...
|---|---|
| Package contracts | Complete portable source archives for frontend, formatter, semantics, and (so far, `add`/`fmt`/`init`/`remove`/`tree`/`why` only) cli, with versioned registry dependencies, exact locks, and declared local overrides |
