...
let makeBinding name =
    LetBindingSyntax(name = name, value = ExprInt(
        0
...
let boxType =
    TypeDecl(name = "Box", typeParameters = [], constructors = [makeConstructor("Wrap"), makeConstructor(
        "Empty"
    )], isRecord = false, derivingTraits = [])

let compatibilityProgram =
    ProgramSyntax(items = [TopLevelLet(makeBinding("run"))(false), TopLevelRecursiveGroup(
        [makeBinding("even"), makeBinding("odd")]
    ), TopLevelType(boxType), TopLevelTypeAlias(
        TypeAliasDecl(name = "Alias", typeParameters = [], target = TypeNamed("Int"))
    ), TopLevelZeroCostType(
        ZeroCostTypeDecl(name = "UserId", typeParameters = [], constructor = makeConstructor(
            "UserId"
        ), derivingTraits = [])
    ), TopLevelTrait(TraitDecl(name = "Display", typeParameters = [], supertraits = [], methods = [])), None
...
let checkCompatibilityInterface unit =
    ...
    |> assertNamed(
        "compatibility",
        Ok(
            ModuleImportInterface(name = "Example", exports = [ImportValueExport("run"), ImportValueExport(
                "even"
            ), ImportValueExport("odd"), ImportTypeExport("Box"), ImportConstructorExport(
                "Wrap"
...
let explicitProgram =
    ProgramSyntax(items = [TopLevelExport(
        ExportDecl(items = [ExportValue("run"), ExportType(
            "Box",
            ExportConstructorsSelected(["Wrap"])
        ), ExportModule("Internal")])
    ), TopLevelLet(makeBinding("run"))(false), TopLevelLet(
        makeBinding("privateValue"),
        false
    ), TopLevelType(boxType)], body = None)
...
let checkExplicitInterface unit =
    ...
    |> assertNamed(
        "explicit",
        Ok(
            ModuleImportInterface(name = "Example", exports = [ImportValueExport("run"), ImportTypeExport(
                "Box"
            ), ImportConstructorExport(
                "Wrap"
            ), ImportModuleExport(
...
let checkExportPosition unit =
    ProgramSyntax(items = [TopLevelLet(makeBinding("run"))(false), TopLevelExport(
        ExportDecl(items = [ExportValue("run")])
    )], body = None)
    |> buildModuleInterface("Example")([])
...
let checkUnknownConstructor unit =
    ProgramSyntax(items = [TopLevelExport(
        ExportDecl(items = [ExportType("Box")(ExportConstructorsSelected(["Missing"]))])
    ), TopLevelType(boxType)], body = None)
    |> buildModuleInterface("Example")([])
...
let checkDuplicateExport unit =
    ProgramSyntax(items = [TopLevelExport(
        ExportDecl(items = [ExportValue("run"), ExportValue("run")])
    ), TopLevelLet(makeBinding("run"))(false)], body = None)
    |> buildModuleInterface("Example")([])
