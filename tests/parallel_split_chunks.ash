// expect: 4|9|27
// CO-18: Ashes.Parallel.splitChunks(bytes)(sep)(n) splits a byte buffer into up to n contiguous
// (bytes, lo, hi) chunks at record-separator (here newline = 10) boundaries, so no record straddles
// a chunk. Fed straight to Ashes.Parallel.reduce it keeps the parallel fold at the caller's concrete
...
let chunks = Ashes.Parallel.splitChunks(bytes)(10)(4)

let addI a b = a + b

let nChunks = len(chunks)(0)

let totalZ = Ashes.Parallel.reduce(addI)(0)(foldZ)(chunks)

let covered = Ashes.Parallel.reduce(addI)(0)(foldSpan)(chunks)
