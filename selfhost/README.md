...
| Backend | `selfhost/packages/backend`'s `AshesCompiler.Backend.Llvm` binds a growing subset of the LLVM C API (context/module/builder/type/value/basic-block/global creation, arithmetic/comparison/multi-arm switch dispatch, control flow with the no-`phi` alloca/store/load slot pattern, function calls, and object/assembly emission with host-CPU-tuned target machines), proven end to end by `selfhost/tests/backend`'s hand-built test programs, checked with `readelf` and independent exact-instruction assembly dumps. `AshesCompiler.Backend.IrCodegen` is the first genuinely IR-driven slice: it walks a REAL `IrFunction` (produced by running actual source through the self-hosted `Frontend`/`Semantics` packages) and drives the LLVM bindings from its real instructions, currently covering scalar arithmetic, locals, control flow, and `PrintInt` (`LoadConstInt`/`MulInt`/`AddInt`/`SubInt`/`CmpIntGt`/`StoreLocal`/`LoadLocal`/`Label`/`Jump`/`JumpIfFalse`/`Return`/`PrintInt`, enough for a plain `if`/`then`/`else` and printing a computed `Int`), with a zero-field `AllocAdt` (a stack slot standing in for real arena allocation) plus, now, a real `malloc`-backed `AllocAdt`/`SetAdtField` for any field-carrying constructor (`runtimeManaged = fieldCount > 0`, a conservative RC-by-default classification — verified byte-exact via assembly dump: real 16-byte RC header, tag/field words at the documented offsets), and arena bookkeeping instructions otherwise treated as explicit no-ops rather than real scoped-arena codegen. `CoreLowering.ash` now emits a single, non-cascading `RcDrop` for a top-level RC-managed binding that is never referenced again (a narrow, always-safe case — cascading drops, multi-argument constructors, and shadowing-aware liveness remain unimplemented); `IrCodegen`'s `RcDrop` codegen decrements the header and calls `free` only at zero, verified via a real-IR test and a byte-exact assembly dump. `IrCodegen` also gained `GetAdtField`/`GetAdtTag` (the read half of `SetAdtField`/`AllocAdt`'s tag write), verified via a hand-built `IrFunction` since no real source could drive them yet at the time — real `match` still needs a null-representable-type check and per-arm RC cleanup not yet built. `CoreLowering.ash` now lowers a top-level `type` declaration — including a generic one — into a real constructor layout, so a user-defined record (`type Point = | x: Int | y: Int`, `Point(x=3, y=4)`, `p.x`), a multi-constructor ADT (`type Shape = Circle(Int) | Square(Int)`), and a generic type (`type Box(a) = | value: a`, instantiated as both `Int` and `Str`) all compile through the complete pipeline and run on a real Linux process — the first user-defined type this compiler has taken from source to a running executable. `IrCodegen` also gained `LoadConstStr`/`PrintStr`, closing the last gap in `Ashes.IO.print`'s existing `SemString` dispatch (a string literal's static storage matches `LlvmCodegenMemory.cs`'s real layout exactly — the same 16-byte RC header as any `AllocAdt` cell, plus a length word, so no string-specific `RcDrop` case is needed); this required teaching `AshesCompiler.Backend.ElfLinker` to resolve absolute `.text` references into an object's own `.rodata` section (a relocation shape no prior instruction produced) via a second, page-aligned read-only `PT_LOAD` segment, and to apply relocations whose patch site lies inside `.rodata` itself (`.rela.rodata`: the absolute `.text` block addresses of the `switch` jump table LLVM emits for a six-plus-constructor `match`). `Ashes.IO.print("hello")` now compiles through the complete self-hosted pipeline and runs on a real Linux process, printing `hello` — the first string literal this compiler has taken from source to output. `IrCodegen` also gained `CmpIntEq`/`CmpIntNe` and the `Borrow`/`CopyOutArena` instruction cases (both plain SSA-value aliases), which turned out to be the entire gap blocking real `match`: `CoreLowering.ash` already emitted a complete `match` compilation (null-pointer guard, tag comparison, field extraction, per-arm `RcDrop`) for any registered constructor, intrinsic or user-defined — only codegen support for that instruction shape was missing. `match x with | Some(v) -> print(v) | None -> print(0)` and a user-defined multi-constructor `match` both now compile through the complete pipeline and run on a real Linux process, reading a pattern-matched field back for the first time. `IrCodegen` also gained `SwitchTag`/`IrSwitchCase` support for tag-GROUPED matches (3+ arms sharing a constructor tag, e.g. nested patterns like `Some(Some(v))`/`Some(None)`), fixing a real, reproducible memory-corruption bug found and root-caused along the way: the original single-pass implementation interleaved a `labelBlocks` lookup with the `LLVMAddCase` FFI call per case, and a lookup performed AFTER an `addCase` FFI call reliably read back wrong data for later entries in the same list. The fix (`resolveSwitchCases`/`addResolvedSwitchCases`) resolves every case's block first in a pass with no FFI calls, then applies `addCase` in a second pass — no lookup ever follows an FFI call. `match x with | Some(Some(v)) -> print(v) | Some(None) -> print(-1) | None -> print(0)` on `Some(Some(7))` now compiles through the complete pipeline and prints `7`. The entry function's `Return` lowers unconditionally to a raw Linux `exit` syscall plus `buildUnreachable` rather than `ret`, matching the real linker's `e_entry` contract (the entry function's own machine code is the literal OS process entry point, with no return address on the stack); `PrintInt` reuses the same inline-assembly syscall mechanism for a raw `write`. `AshesCompiler.Backend.ElfLinker` links objects into real linux-x64 executables (pure Ashes byte manipulation, no `ld`/`lld`), choosing automatically between two paths: a real `.ash` **file** (`Ashes.IO.print(42 - 84)`) has been compiled through the complete self-hosted pipeline, statically linked, `chmod +x`'d, and spawned by the automated test suite itself, producing real stdout (`-42`) and exit code `0`; separately, the linker also now supports dynamic linking — `.text` relocations against a known external symbol (`malloc`/`free`) link a real dynamically-linked executable (`jmp`-through-GOT stubs, `.dynamic`/hash/`.dynstr`/`.dynsym`/`.rela.dyn`), verified via a hand-built module and `strace`: the kernel loads the real `ld-linux-x86-64.so.2`, which loads real glibc and calls its actual `malloc` (a genuine `brk` syscall), before the program's own `exit(0)` fires — the first Linux `ld.so`-loaded, real-libc-calling executable this compiler has produced (`IrCodegen` doesn't call `malloc`/`free` from real IR yet, so this proves the linker mechanism alone via a hand-built module, not an end-to-end `.ash` file). Landing the print path also required a genuinely separate fix in `AshesCompiler.Semantics.CoreLowering`: builtin resolution is now intrinsic (`standardBuiltinLayouts`/`standardConstructorLayouts`, seeded automatically by `initialState`), matching language.md's "qualified access (no import required)"; `standardConstructorLayouts` now also covers `Maybe`'s `None`/`Some` and `Result`'s `Ok`/`Error` on the same "always available, no import required" basis. `IrCodegen` also gained `LoadConstBool`, needed for the new `&&`/`||` short-circuit operators (`CoreLowering.ash` desugars both to `lowerIf`, so a literal `true`/`false` for the elided branch is unavoidable): every Bool value is represented the same as every other scalar here, a canonical 0/1 `i64` (`AshesCompiler.Backend.Llvm` gained `buildZExt`/`int1Type` alongside the existing `buildTrunc`), zero-extended right after `CmpIntGt`/`CmpIntEq`/`CmpIntNe`'s native `i1` `icmp` result and truncated back to `i1` only at `JumpIfFalse`'s `buildCondBr`, the one place LLVM actually requires the narrower type — this keeps a Bool value round-tripping through a `StoreLocal`/`LoadLocal` slot exactly like `&&`/`||`'s desugared `if` branches need, with no bits lost. `Ashes.IO.print(if true && false || true then 1 else 0)` now compiles through the complete self-hosted pipeline and runs on a real Linux process, printing `1`. `AshesCompiler.Backend.ElfLinker` can now link an object needing BOTH the dynamic-import path (`malloc`/`free`) AND a `.rodata` segment (a string literal) together — previously a clean `Error`, since any program that both allocates a heap record and prints a string literal hits this shape. Fixing it surfaced a second relocation shape: the same string-length load that compiles to an absolute `R_X86_64_32S` reference alone compiles to a PC-relative `R_X86_64_PC32` once the function also calls `malloc`/`free`; `DataRelocationPatch` now records which formula (`S + A` or `S + A - P`) a patch needs, and `linkWithDynamicImports` gained a third, page-aligned, read-only `PT_LOAD` segment for `.rodata` alongside its existing dynamic-linking data segment. `let x = Some(42)\nAshes.IO.print("hello")` now compiles through the complete self-hosted pipeline and runs on a real Linux process, printing `hello`. Two distinct string literals in one object, a string read back out of a user-defined record field via `match`, and a string reached through a `let` binding (rather than passed directly as `print`'s own argument) all now work end to end too — the last of those needed a genuinely new linker fix: `objdump -dr` showed an 8-byte absolute `R_X86_64_64` relocation (`movabs $imm64, reg`) neither prior fix produced, so `DataRelocationPatch` gained `dataPatchWidth` and `applyDataPatches` now writes the full 64-bit address for that type instead of truncating to 32 bits. `IrCodegen` also gained `PanicStr` — `Ashes.IO.panic` was already an intrinsic builtin lowering to it, only codegen was missing — reusing the same message-printing helper `PrintStr` now shares (`emitPrintStrBytesWithNewline`) before exiting `1` instead of `0`; `Ashes.IO.panic("boom")` now compiles through the complete self-hosted pipeline and runs on a real Linux process, printing `boom` and exiting `1`. `IrCodegen` also gained `CmpStrEq`/`CmpStrNe` (`==`/`!=` on `Str`, and the same instructions string-literal `match` patterns use): a length check first, then a real libc `memcmp` call over the payload bytes once lengths match, needing only a one-line addition to `ElfLinker`'s own recognized-symbol table (`memcmp`, alongside `malloc`/`free`) and no new linker mechanism. `Ashes.IO.print(if "hello" == "hello" && "hello" != "world" && "ab" != "abc" then 1 else 0)` now compiles through the complete self-hosted pipeline and prints `1`. `IrCodegen` also gained `ConcatStr`/`ConcatStrN` (`+` on two `Str` operands, and the N-ary form `IrOptimizer.ash`'s `foldConcatStrChains` folds real chains into as its very last pass): one real `malloc`'d RC-managed `Str` for the sum of every part's length, then each part's payload bytes copied into position via a real libc `memcpy` per part — a fourth recognized `ElfLinker` dynamic-import symbol (`memcpy`, alongside `malloc`/`free`/`memcmp`). `Ashes.IO.print("hel" + "lo " + "world")` now compiles through the complete self-hosted pipeline and prints `hello world`. `IrCodegen` now compiles a whole `IrProgram` (`codegenProgram`), not just its entry function: every lifted helper function (top-level `let f x = ...`, lambdas, curried partial applications) becomes a real `i64 label(i64 env, i64 arg, i64 flag)` LLVM function with stage 0's exact calling convention, with `MakeClosure`/`MakeClosureStack` (the real 32-byte closure object), `CallClosure` (indirect), `CallKnown` (direct, the optimizer's devirtualized form), `LoadFuncAddr`, `LoadEnv`, `LoadArgumentOwnership`, `Alloc`/`AllocStack`, and `StoreMemOffset`/`LoadMemOffset` all supported; `ElfLinker` correspondingly resolves `call`s to, and address-taking of, locally-defined `.text` symbols against `.text`'s own base address, and now places stage 0's 20-byte process-entry trampoline at the start of `.text` on both link paths so the entry function is reached by a real `call` with the stack alignment LLVM-compiled code assumes (previously `e_entry` jumped straight to it, leaving every frame and libc call misaligned by 8). `IrCodegen` also covers every remaining scalar integer instruction ordinary source lowers to — `DivInt`/`DivUInt`, `AndInt`/`OrInt`/`XorInt`, `ShlInt`/`ShrInt` (amount masked, logical right shift), the seven missing comparison predicates, and `PrintBool` (`true`/`false` via the raw `write` syscall from a stack buffer) — so `(1 << 4) + (256 >> 2) + (10 / 3) + (6 & 3) + (6 | 1) + (6 ^ 3)` prints `97` and `print(1 > 2)` prints `false` through the complete self-hosted pipeline. `AshesCompiler.Semantics.ShippedModuleStitching` resolves a program's `import Ashes.*` header against the shipped `lib/Ashes` module texts (transitive closure, header split, interface, `buildModulePlan` order, `stitchProjectSyntax`), so `import Ashes.Collection.List` / `print(Ashes.Collection.List.length([1, 2, 3]))` now compiles through the complete self-hosted pipeline and prints `3` — the first standard-library source function taken from `lib/Ashes` to output; imports of intrinsic-only modules (`Ashes.Byte`, `Ashes.Task`, ...) and bare qualified references without an import are not resolved yet. `let inc x = x + 1` / `print(inc(41))`, the curried `let add x y = x + y` / `print(add(40)(2))`, and `let recursive fact n = ...` / `print(fact(5))` all now compile through the complete self-hosted pipeline (with and without `IrOptimizer`) and run on a real Linux process, printing `42`/`42`/`120` — the first programs with functions beyond the entry this compiler has taken from source to output. `IrCodegen` now emits native tail calls: a `CallKnown` whose result the function immediately returns — directly, or through the `StoreLocal`-into-the-join-slot/`Jump`-or-fallthrough shape every lowered multi-arm body converges on — is fused into a `musttail` call plus `ret` when nothing in the function stack-allocates (an advisory `tail` marker otherwise), so deep self-recursion, mutual recursion, and closure-helper loops run in constant stack without loop conversion (`tests/arena_chunk_growth_stress.ash`, the `mutual_recursion_tco_*` trio, and the `closure_local_helper_*_captures_direct_call` pair all pass through the self-hosted CLI). `TextFromInt` (a fresh RC heap string built from the `PrintInt` digit phases), `TextByteLength`, and `BytesLength` (the view-flag-masked `len` word) codegen end to end, with `standardBuiltinLayouts` schemes making `Ashes.Text.fromInt`/`Ashes.Text.byteLength`/`Ashes.Byte.length` the first qualified builtin members reachable beyond the original `Ashes.IO` set; the byte-level set followed (`Ashes.Byte.fromText`/`get`/`indexOf`/`compare`/`subText`/`subView` and `Ashes.Number.UInt.toInt`), and `subView`'s string VIEWS (`{len|VIEW, backingBytesAddr}`) made every byte consumer (`ConcatStr` parts, `CmpStrEq`, `PrintStr`/`PanicStr`, length reads) go through one view-aware `emitStringParts` helper; the UTF-8 pair (`Ashes.Text.unconsText`, `Ashes.Rune.toText`) closed the builtin set `lib/Ashes/Text.ash`'s own body uses, leaving only `join`'s both-operands-unresolved `+` (eager Int defaulting in a nested recursive group) before `import Ashes.Text` programs lower. See `docs/md/future/SELF_HOSTING.md`'s "LLVM code generation and runtime integration" checklist for the current surface and exact scope boundary. No optimization-level selection, and closure drop, non-trivial ADTs, RC/drop/reuse codegen, real arena/region codegen, TLS sections, the other three target RIDs, and the rest of the LLVM C API surface are unported |
