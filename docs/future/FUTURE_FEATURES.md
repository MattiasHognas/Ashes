...
| [Unified Capabilities](UNIFIED_CAPABILITIES.md) | Largely complete | Effects renamed to capabilities (`effect`→`capability`, `uses`→`needs`); dynamic `handle`; static `provide` for concrete instances (the full DI story) **and generic functions monomorphized per concrete use** (`display(42)`/`display(true)` resolve to different providers). Provider/handler ambiguity (`ASH027`) and duplicate/incomplete-provider (`ASH026`) checks. See [LANGUAGE_SPEC.md](../LANGUAGE_SPEC.md) §20. **Remaining**: *recursive* and *higher-order* generic uses (a `Cap.op` inside a closure passed to another function) — needs dictionary passing; plus provider import/export across modules |
