...
let checkDefaults unit =
    ...
    |> assertNamed(
        "defaults",
        Ok(
            ProjectManifest(entry = "src/Main.ash", name = None, namespace = None, version = None, sourceRoots = ["."], includeRoots = [], outDir = "out", target = None, defaults = emptyDefaults, dependencies = [], devDependencies = [], overrides = [])
        )
    )
...
let checkCompleteManifest unit =
    ...
    |> assertNamed(
        "complete",
        Ok(
            ProjectManifest(entry = "Main.ash", name = Some("demo"), namespace = Some("Demo"), version = Some(
                "1.2.3"
            ), sourceRoots = ["src", "generated"], includeRoots = ["vendor"], outDir = "build", target = Some(
                "linux-x64"
            ), defaults = ProjectDefaults(optimize = Some(
                true
            )), dependencies = [ProjectDependency(name = "json", source = RegistryDependency(
                "^1.2.0"
            )), ProjectDependency(name = "local", source = PathDependency(
                "../local",
                Some("LocalApi")
            ))], devDependencies = [ProjectDependency(name = "checks", source = PathDependency(
                "../checks",
                None
            ))], overrides = [ProjectOverride(name = "json", path = Some(
                "../json"
            )), ProjectOverride(name = "broken", path = None)])
        )
    )
...
let checkPermissiveArrays unit =
    ...
    |> assertNamed(
        "permissive arrays",
        Ok(
            ProjectManifest(entry = "Main.ash", name = None, namespace = None, version = None, sourceRoots = ["src"], includeRoots = [], outDir = "out", target = None, defaults = emptyDefaults, dependencies = [], devDependencies = [], overrides = [])
        )
    )
...
let checkIgnoredUnsupportedDependency unit =
    ...
    |> parseProjectManifest
    |> assertNamed(
        "ignored unsupported dependency",
        Ok(
            ProjectManifest(entry = "Main.ASH", name = None, namespace = None, version = None, sourceRoots = ["."], includeRoots = [], outDir = "out", target = None, defaults = emptyDefaults, dependencies = [], devDependencies = [], overrides = [])
        )
    )
