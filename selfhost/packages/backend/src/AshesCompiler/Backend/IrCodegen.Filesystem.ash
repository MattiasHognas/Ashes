...
// `File.exists`/`open`/`readChunk`/`close`/`writeText`/`replace` and `Directory.createAll` over
// raw syscalls, `Directory.entries`/`removeTree` over the libc dynamic-import surface
