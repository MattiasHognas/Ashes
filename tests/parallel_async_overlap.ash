...
// A `both` worker makes progress while an async I/O is live. Under Ashes.Task.all the scheduler
...
let taskA =
    async(match await Ashes.Net.Http.get("http://127.0.0.1:__TCP_PORT__/x") with
        | Ok(t) -> tlen(t)(0)
        | Error(_) -> -1)

let taskB =
    async(match await Ashes.Task.task(0) with
        | Error(_) -> 0
        | Ok(_) ->
