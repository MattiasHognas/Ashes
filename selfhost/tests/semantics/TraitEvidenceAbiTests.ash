...
let binaryMethod name result =
    TraitMethodDecl(name = name, signature = TypeArrow(TypeNamed("a"))(TypeArrow(TypeNamed("a"))(result)([])(None))([])(None), defaultImplementation = None)

let equalTrait unit = TraitDecl(name = "Equal", typeParameters = [TypeParameter(name = "a")], supertraits = [], methods = [binaryMethod("notEqual")(TypeNamed("Bool")), binaryMethod("equal")(TypeNamed("Bool"))])

let orderedTrait unit = TraitDecl(name = "Ordered", typeParameters = [TypeParameter(name = "a")], supertraits = [TraitConstraintSyntax(traitName = "Equal", typeArguments = [TypeNamed("a")])], methods = [binaryMethod("greater")(TypeNamed("Bool")), binaryMethod("compare")(TypeNamed("Int"))])
...
let expectDeterministicDictionaryAbi unit =
    ...
    |> TopLevelTrait], body = None)) with
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = environment, error = None } ->
            match planTraitEvidenceAbi([TraitConstraint(traitName = "Ordered", typeArguments = [SemInt]), TraitConstraint(traitName = "Equal", typeArguments = [SemString])])(environment) with
        ...
        | ProgramInferenceResult { semanticType = _semanticType, substitution = _substitution, environment = _environment, error = Some(error) } -> test.fail("trait declarations should infer: " + Ashes.Trait.Show.show(error))
