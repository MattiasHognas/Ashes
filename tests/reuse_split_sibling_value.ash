...
let recursive verify i bad =
    ...
    then bad
        ...
        let key = "k" + Ashes.Text.fromInt(i)
        in
            match Ashes.Collection.HashTrie.getHashed(Ashes.Collection.HashTrie.hashText(key))(key)(t) with
                | Some((a, b)) ->
                    if a == i + 1000
                    then
                        if b == i
...
Ashes.IO.writeLine("bad=" + Ashes.Text.fromInt(verify(0)(0)) + " n=" + Ashes.Text.fromInt(Ashes.Collection.HashTrie.size(t)))
